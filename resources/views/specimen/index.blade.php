    ...
    <form id="editpatient">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newTicketModalLabel">Edit specimen Info</h5>
              ...
              <div class="modal-body">
                ...
                <input type="hidden" type="text"  id="edit_userid_id">
                  <div class="form-group">
                    <div class="form-group">
                        <label for="description" class="mr-1">Specimen Name </label>
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="update-specimen btn btn-primary">Sava</button>
                <p class="success"></p>
            </div>
...
<div class="modal-dialog modal-lg" role="document">
    ...
    <div class="modal-header pd-x-20">
        <h6 class="tx-14 mg-b-0 tx-uppercase tx-inverse tx-bold">Add Specimen</h6>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
        <span aria-hidden="true">&times;</span>
        </button>
    </div>
    ...
    <div class="modal-body pd-20">
        <div class="form-group">
            <div class="form-group">
                <label for="description" class="mr-1">Specimen's Name </label>
...
<script type="text/javascript">
    ...
    $(document).on('click','.edit-specimens',function(e){
        e.preventDefault();
        var specimen_id=$(this).val();
        $('#newTaskModal').modal('show');
        $.ajax({
            type:"POST",
            url:"/edit-specimen/"+specimen_id,
            success:function(response){
                $('#edit_userid_id').val(response.specimen.id);
                $('#newname').val(response.specimen.name);
                if(response.specimen.status==1){
     ...
     $(document).on('click','.update-specimen',function(e){
        $.ajaxSetup({
            headers: {
        ...
        e.preventDefault();
        var specimen_id= $('#edit_userid_id').val();
        ...
        $.ajax({
            type: "PUT",
            url:"/update-specimen/"+specimen_id,
            data: data,
            datatype:'json',
            success: function(response){
                //alert(data);
    ...
    $(document).on('click','.delete-specimens',function(e){
        $.ajaxSetup({
    ...
    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $.ajax({
                 type: "POST",
                 url:"/delete-specimen/"+specimen_id,
                 datatype:'json',
    ...
    headers: {
        ...
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
         ...
         $(this).html('Sending..');
                 $.ajax({
                 data: $('#specimenForm').serialize(),
                 url:  "{{ route('store.specimen') }}",
                 type: "POST",
                 datatype:'json',
                 success: function(data){
                     //alert(data);
                     $('#specimenForm').trigger("reset");
